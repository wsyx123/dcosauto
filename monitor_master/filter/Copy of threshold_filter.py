...
class ThresholdFilter(object):
    ...
    def cpu(self,usage,date):
        ...
        if usage >= float(threshold_dict['begin_warning']) and usage < float(threshold_dict['end_warning']):
            problem_data['name'] = u'cpu utilization:{}%'.format(usage)
            problem_data['host'] = str(self.host)
            problem_data['time'] = date
            
            message = alarm_msg_template.email_template.format(date,self.host,problem_data['name'],'warning')
            content = notifyDetail_format.notify_detail_content.format(date,self.host,problem_data['name'],'warning')
            send_email_result = email_way.send_email_dispatch(to='18508503875@139.com', subject=u'cpu 利用率告警', message=message)
            if send_email_result['status']:
                notify_detail_data = notifyDetail_format.generate_detail_data("Email", "cpu 利用率告警", content, "18508503875@139.com", "SEND","")
                notifyDetail_format.write_notify(notify_detail_data)
            else:
                notify_detail_data = notifyDetail_format.generate_detail_data("Email", "cpu 利用率告警", content, "18508503875@139.com", "UNSEND",send_email_result['msg'])
                notifyDetail_format.write_notify(notify_detail_data)
