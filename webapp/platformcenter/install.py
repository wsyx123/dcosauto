...
#         if saveobj.save():
    ...
    return render_to_response('platform/custom_install.html',{'hosts':hosts,'templates':templates})

def auto_install(request):
    hosts = platformhosts.objects.all()
    templates = platformtemplate.objects.all()
    return render_to_response('platform/auto_install.html',{'hosts':hosts,'templates':templates})